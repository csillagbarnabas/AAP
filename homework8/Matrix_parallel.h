...
Matrix<T> operator*(Matrix<T> const& A, Matrix<T> const& B)
{
	auto f1 = [&](int i, int k,int j0, int j1){		
		double x = 0.0;
		for(int j=j0;j<j1;j++)
		{
			x += A(i,j)*B(j,k);
	...
	auto f2 =[&](int i, int k)
	{
		int number_of_threads = (int)std::thread::hardware_concurrency();
		auto n = std::thread::hardware_concurrency();
		std::vector<std::future<double>> futures(n);
		for(int l=0;l<number_of_threads;++l){
			auto it0 = l*B.Nsize()/n;
			auto it1 = (l+1)*B.Nsize()/n;
			futures[l] = std::async(f1, i, k, it0,it1);
		}
		auto parallel_res = std::accumulate(futures.begin(), futures.end(), 0.0, [](double a, std::future<double>& f){ return a + f.get();});
		return parallel_res;
	};
	return Matrix<T>(Idx2{},f2,A.Nsize());
}
template<typename T>
Matrix<T>&& operator*(Matrix<T> && A, Matrix<T> const& B)
{
	...
	Matrix<T> result(Idx2{},f2,A.Nsize());
	A=std::move(result);
	return std::move(A);
}
template<typename T>
